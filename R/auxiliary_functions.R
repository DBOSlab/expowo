...
.getgenURI <- function(family = family,
  ...
  for (i in seq_along(powo_codes$uri)) {
    ...
    powo_fams_uri[[i]] <- readLines(textConnection(httr::content(httr::GET(powo_codes$uri[i]), as="text")), encoding="UTF-8", warn=F)
    ...
    if (length(temp) != 0) {
      ...
      if (any(!new_fam_name %in% powo_codes$family)) {
        ...
        httr::set_config(httr::config(ssl_verifypeer = FALSE, ssl_verifyhost = FALSE))
        powo_fams_uri[[i]] <- readLines(textConnection(httr::content(httr::GET(new_powo_fam_uri), as="text")), encoding="UTF-8", warn=F)
...
.getsppURI <- function(powo_codes,
  ...
  for (i in seq_along(powo_codes$powo_uri)) {
    ...
    httr::set_config(httr::config(ssl_verifypeer = FALSE, ssl_verifyhost = FALSE))
    powo_genus_uri[[i]] <- readLines(textConnection(httr::content(httr::GET(powo_codes$powo_uri[i]), as="text")), encoding="UTF-8", warn=F)
...
.getNumb <- function(df,
  ...
  for (i in seq_along(df$powo_uri)) {
    ...
    tryCatch({
      ...
      httr::set_config(httr::config(ssl_verifypeer = FALSE, ssl_verifyhost = FALSE))
      list_html[[i]] <- readLines(textConnection(httr::content(httr::GET(paste(df$powo_uri[i])), as="text")), encoding="UTF-8", warn=F)
      # Adding a counter to identify each running search
