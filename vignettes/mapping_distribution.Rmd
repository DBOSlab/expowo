...
package's `powoSpecies` function to get distribution information for the species of the target family or genus. It is also possible to adjust one of 
...
resulting object has to be as input of the argument `botctrs` within the `powoMap` function. Then, use the previously generated `mapspdist` object as input data of the `powoMap` function and set the `distcol` argument as "native_to_botanical_countries".
...
\
\
To create global maps of Lecythidaceae, change the argument `distcol` to  
"native_to_botanical_countries", the `vir_color` argument to "viridis", and 
