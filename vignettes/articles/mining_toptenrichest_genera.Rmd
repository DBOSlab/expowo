...
fam <- c("Bignoniaceae", "Solanaceae", "Lecythidaceae")
...
toptenGen(powocodes$family, powocodes$uri,
          ...
          filename = "Bigno_Lecyt_Solanaceae_search")
```
...
fams <- c("Begoniaceae", "Solanaceae", "Lecythidaceae")
gens <- angioData$genus[angioData$family %in% fams]
species_number <- table(gens)
species_number <- as.data.frame(species_number)
family <- angioData$family[angioData$family %in% fams]
table <- data.frame(family, gens)
teste <- merge(species_number, table, by = "gens")
teste <- unique(teste)
test <- teste %>% arrange(desc(teste$Freq)) %>% group_by(family) %>% 
  slice(1:10)
species_number <- test$Freq
family <- test$family
genus <- test$gens
res <- data.frame(family, genus, species_number)
