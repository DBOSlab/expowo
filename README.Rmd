...
```{r}
library(expowo)
powocodes <- taxize::get_pow(c("Typhaceae", "Lecythidaceae"))
powocodes <- data.frame(powocodes)
powocodes <- cbind(family = c("Typhaceae", "Lecythidaceae"), powocodes)
powoGenera(dir = "results_powoGenera/",
           filename = "Typhaceae_Lecythidaceae",
           powocodes$family,
           powocodes$uri,
           verbose = TRUE,
           save = FALSE)
...
``` {r}
powocodes <- taxize::get_pow("Araceae")
powocodes <- data.frame(powocodes)
powocodes <- cbind(family = "Araceae", powocodes)
...
of any target plant family. The following example shows the top ten of the plant 
family "Lecythidaceae"!

##### Example of a POWO search with `toptenGen`:

```{r}
powocodes <- taxize::get_pow("Lecythidaceae")
powocodes <- data.frame(powocodes)
powocodes <- cbind(family = "Lecythidaceae", powocodes)
