...
The `powoMap` can use as input data the dataframe-formatted resulting query from `powoSpecies` function or a spreadsheet with some specifications (see below). Then, `powoMap` can automatically create a single global
...
package's  associated `powoSpecies` function to get distribution information for the species of the target family or genus. It is also possible to adjust one of 
...
# 2. Using an external spreadsheet as input data in powoMap

It is possible to use data from other databases to create global maps using 
our `powoMap` function. The spreadsheet with the data extracted from other sources must have standardized columns in order to the `powoMap` function works properly (see the example above as the ideal structure). Therefore, the spreadsheet to be used as input must have at least these four columns: "genus", "taxon_name", "native_to_country", and "native_to_botanical_countries". The "genus" column have to contain each genus name written in full (e.g., Aphelandra) stored in each row, "taxon_name" refers to the scientific plant name without the authorship (e.g., Aphelandra acrensis), the "native_to_country" column must have the full country name of the global native distribution of each species, with each political country name separated by a comma and space right after (e.g., Brazil, Peru), and the "native_to_botanical_countries" column refers to the global native distribution according to the TDWG botanical subdivision level 3 of each species (e.g., Brazil North, Peru). 

\

# 3. Creating global-scale map for one genus to show species richness at political country level
...
# 4. Creating global-scale map for one genus to show species richness at botanical country level
...
# 5. Creating global-scale map of species richness for an entire family
